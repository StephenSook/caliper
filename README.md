...
Every figure above is computed by the code in this repository from the supplied
export, and you can watch it recompute two ways. Open
[the live instance](https://caliper-77ma.onrender.com) and press **Run the
audit**, which needs no clone, no install and no credential. Or clone this and
run `python -m caliper.instrument.audit` against a copy of the export, pointed
at by `CALIPER_DATA_DIR`.

The export itself is competition confidential and is deliberately not committed,
so that second command has nothing to read until you supply it, and it says so
and exits rather than failing at you. The statistics are covered without the
package either way: `pytest` checks every one against hand computed fixtures.
